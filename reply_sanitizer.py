...
# 她刚拒绝了、或者说自己累了——认出这个，别让他换个说法再要一次。
#
# 真事（2026-09-23）：
#   10:49 他「都做了什么，挑几个跟我炫一下」
#   11:09 他「那张卡给我看看」→「一张张拍给我」
#         她「一张照片拍不下」「不给」「累死闪闪」   ← 拒绝了三次
#   11:10 他「那你就挑你最得意的那几件念给我听」   ← 把 10:49 那句又要了一遍
# 她：「我刚刚说过的话居然要在问我一遍」。
#
# ⚠️ 我先试过从**他**那端查重（比他这条跟最近说过的话像不像）——
# 那两句的字面相似度只有 0.207，用词完全不同、意思一样，字面比对根本抓不到。
# 抓不到的护栏比没有更坏，所以删了，改从**她**这端认信号：
# 「不给」「累死了」这种词是确定的，不用猜语义。
_DECLINE_RE = re.compile(
    r"(不给|不想给|不发|不拍|拍不下|拍不了|做不到|不行了|算了|不要了|"
    r"我不|别问了|不想说|懒得|没力气|没精力|"
    r"累死|好累|太累|累了|困死|烦死|不想动|动不了)")


def sounds_like_decline(text: str) -> bool:
    """她这条里有没有「我拒绝 / 我累了」的信号。

    ⚠️ 疑问句不算（「你不给我吗？」是在问，不是在拒），
    她说别人累也不算（「她好累」）——宁可漏判，不可误判：
    判错的代价是他该追的时候不敢追了。
    """
    t = (text or "").strip()
    if not t or t.endswith(("吗", "嘛", "么", "?", "？")):
        return False
    if re.search(r"(他|她|它|他们|她们)\s*(好|很|太)?(累|烦|困)", t):
        return False
    return bool(_DECLINE_RE.search(t))
