...
# 短名 -> 完整模型 id，给 /model 命令在聊天里随手切，不用改 Render 重启
MODEL_ALIASES = {
    "opus": "claude-opus-4-6",       # 一直好用、在 Telegram 上也便宜
    "sonnet": "claude-sonnet-4-6",   # 更快更省
}
...
MAX_TOKENS = int(os.environ.get("OMBRE_BOT_MAX_TOKENS", "3500"))  # 提高些，长回复（涩文等）不被截断；Telegram 人设短，不肉疼
...
- 思考用中文。
- 连发：可以把话拆成几条短消息连着发，每条之间用一个 ‖ 隔开（像真人发微信：先应一声、再接正事）。别硬凑，一句能说清就一句，不必每次都连发。
...
current_model: str = MODEL  # 当前模型，可用 /model 在聊天里临时切，落盘后重启也保留
...
async def _send_reply(context, chat_id: int, reply: str, force_voice: bool = False) -> None:
    ...
    if want_voice:
        try:
            audio = await _tts(reply.replace("‖", "，"))  # 语音里 ‖ 当停顿，不念出来
            await context.bot.send_voice(chat_id=chat_id, voice=audio)
            return
        except Exception:  # noqa: BLE001
            logger.exception("TTS 失败，退回文字")
    # 连发：他可以用 ‖ 把话分成几条，逐条发出来（像真人发微信）。只有第一条带时间戳。
    segments = [s.strip() for s in reply.split("‖") if s.strip()] or [reply]
    for idx, seg in enumerate(segments):
        text_out = (_stamp() + seg) if idx == 0 else seg
...
def _load_state() -> None:
    ...
    try:
        ...
        current_model = data.get("current_model") or current_model
        logger.info("已载回 %d 段对话 | 当前模型 %s", len(histories), current_model)
    except Exception:  # noqa: BLE001
...
async def model_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/model 看当前模型；/model opus|sonnet 当场切换（落盘，重启不丢）。"""
    chat_id = update.effective_chat.id
    if ALLOWED_CHAT_IDS and chat_id not in ALLOWED_CHAT_IDS:
        return
    global current_model
    ...
    if not arg:
        cur_short = next((k for k, v in MODEL_ALIASES.items() if v == current_model), current_model)
        await update.message.reply_text(
            f"现在用的是 {cur_short}（{current_model}）。\n"
            "切换：/model opus（细、稳）| /model sonnet（快、省）。"
        )
        return
    if arg in MODEL_ALIASES:
        target = MODEL_ALIASES[arg]
    elif arg.startswith("claude-"):
        target = arg
    else:
        await update.message.reply_text("没认出来。用 /model opus 或 /model sonnet。")
        return
    current_model = target
    _save_state()
    await update.message.reply_text(f"换好了，现在用 {arg}（{target}）。下一句开始生效。")
...
def main() -> None:
    ...
    if app.job_queue:
        # 「她沉默就主动找她」默认关闭（闪闪要求：不要 Telegram 的不理 30 分钟就发消息）。
        # 想重新打开就把环境变量 OMBRE_BOT_NUDGE 设成 1。
        if os.environ.get("OMBRE_BOT_NUDGE", "0").strip().lower() in ("1", "true", "yes", "on"):
