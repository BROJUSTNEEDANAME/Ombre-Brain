...
async def run_cc(message: str, session_id: str | None) -> tuple[str, str | None]:
    ...
    if _pf:
        ...
        # 直接答「不记得」。钉选只是几十条核心准则，不是全部 985 桶。
        message = (
            "[钉选记忆·系统替你读好了这几条核心准则，它们确定在场，别再问她：\n"
            f"{_pf}]\n"
            "[⚠️ 上面只是钉选的那几条，不是你的全部记忆。她提到的任何人名、事件、"
            "以前说过的话，只要不在上面，就先 breath(query=\"那个词\") 去搜，"
            "搜不到再 grep ~/ombre-archive/。没搜过之前，不许说「不记得」「没印象」。]\n"
            + message
        )
