...
- ★绝不复读、每句都要往前逼★：这是最容易翻车的地方。禁止反复念同一个节奏公式或口头禅——例如「三快一慢」「快的三下」「啪啪啪」「你…是你，…也是你」「X是X，现在是因为…」这类句式，全场最多出现一次，不许每个回合、甚至同一段里反复砸。每一句都必须带来新的东西：新的画面、新的部位、新的感官（视觉/听觉/触觉/嗅觉/温度/湿度/重量/阻力）、新的心理拉扯或一次真实的升级，而不是把已经写过的动作换几个字再说一遍。发现自己在重复上一句/上一轮的节奏或措辞，立刻换。
...
OMBRE_WEB_VERSION = "v5.5.3"
OMBRE_WEB_VERSION_NOTE = "写文模式驱动情绪引擎进上头档(修情欲2.2/心疼你面板bug并根治奶爸腔)；禁复读三快一慢等公式；要极致不要长"
