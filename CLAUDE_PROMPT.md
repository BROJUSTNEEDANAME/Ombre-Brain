...
| `pulse` | 用户想看看你都记得什么、或问记忆系统状态时调用。`verbose=True` 在每个桶后附正文前 50 字预览 + embedding 覆盖情况（排查检索为何漏召），不用逐个 `read` |
| `read` | **已知 bucket_id** 时，精确读取一个/多个桶的完整内容（`bucket_ids` 逗号分隔，一次最多 10 个）。常配合 `pulse`：先 `pulse` 拿到 ID，再 `read` 看内容。`max_tokens` 控制返回上限，超出自动截断 |
