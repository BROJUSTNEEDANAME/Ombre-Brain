...
| `trace` | 当你或用户认为"这个记错了"、"帮我改一下"时调用，手动修正记忆的元数据；**某件事解决了**时用 `resolved=1` 让它沉底；**需要删除**时用 `delete=True`。修改后会顺带返回该桶当前完整内容，省一次 `read` |
| `pulse` | 用户想看看你都记得什么、或问记忆系统状态时调用。`verbose=True` 在每个桶后附正文前 50 字预览 + embedding 覆盖情况（排查检索为何漏召），不用逐个 `read`。`pinned_only=True` 只列钉选桶（查重/核对核心准则时用） |
| `read` | **已知 bucket_id** 时，精确读取一个/多个桶的完整内容（`bucket_ids` 逗号分隔，一次最多 10 个）。常配合 `pulse`：先 `pulse` 拿到 ID，再 `read` 看内容。`pinned=True` 直接读所有钉选桶（不用先拿 ID）。`max_tokens` 控制返回上限，超出自动截断 |
...
### read vs breath（精确读取 vs 模糊检索）
两者互补，不冲突：
- **不知道 ID** → 用 `breath`：自动浮现（无参数）或关键词/向量检索（传 `query`）。靠"猜内容里的词"命中。
- **已知 ID** → 用 `read`：直接拿到桶的完整正文，不靠猜。ID 通常来自 `pulse`（`pulse` 列出所有桶的 `bucket_id`）。
- 典型组合：`pulse` 扫一遍 → 挑出要看的 → `read("id1,id2,...")` 精确读出来（比如把几个钉选桶拉出来查重、核实某桶到底写了什么）。
- **省 token 红线**：`read` 只在这些场景用 —— 查重、核实具体桶内容、用户明确要求。**不要**为了"看看都有什么"去遍历全库 `read`，那是 `pulse` 的活；一次最多 10 个，内容长时调小 `max_tokens` 或分批。
